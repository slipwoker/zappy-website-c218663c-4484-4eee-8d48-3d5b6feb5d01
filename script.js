      ...
      if (window.zappyContactFormLoaded) {
        try {
          ...
          const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
            ...
            body: JSON.stringify({
              websiteId: 'c218663c-4484-4eee-8d48-3d5b6feb5d01',
              name: name,
              email: email,
              subject: eventType ? `Contact Form - ${eventType}` : 'Contact Form Submission',
              message: message,
              phone: phone || null
            })
          });
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "יואב ואריאל גינדי משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
